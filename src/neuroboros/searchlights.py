...
TianLUT = pd.read_csv(
    os.path.join(HERE, "support_files", "atlas-TianS1S2S3S4_combined_keylabel.tsv"),
    sep="\t",
)
...
def get_aseg_sls():
    ...
    subcortical_voxelcount = pd.read_csv(
        os.path.join(
            HERE, "support_files", f"atlas-{subcortical_atlas}_VoxelCount.txt"
        ),
        sep="\t",
    )
    ...
    for roi in subcortical_rois:
        nvox = np.array(
            subcortical_voxelcount.VoxelCount[subcortical_voxelcount.Label == roi]
        )[0]
