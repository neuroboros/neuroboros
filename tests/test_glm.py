...
def test_glm():
    ...
    np.testing.assert_allclose(np.mean(R2s), 0, atol=0.001)
    np.testing.assert_allclose(np.mean(betas), 0, atol=0.001)
...
def test_t_value():
    ...
    np.testing.assert_allclose(ts[0], t.statistic, atol=0.001)
